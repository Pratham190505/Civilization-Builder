...
class SecurityController {
  ...
  async createRegionalAdmin(req, res) {
    ...
    try {
      ...
      const { email, password, full_name, mobile, stateId } = req.body;
  ...
  async resetPassword(req, res) {
    try {
      ...
      try {
        await require('../repositories').AuditRepository.create({
          user_id: req.user?.id || 1,
          action: `Reset password for user ID ${id}`,
          entity_name: 'User',
          entity_id: id,
          ip_address: req.ip || req.connection.remoteAddress || '127.0.0.1',
          user_agent: req.headers['user-agent'] || 'unknown',
      ...
      return res.status(500).json({ success: false, message: 'Failed to reset password', errors: [error.message] });
    }
  }
}
