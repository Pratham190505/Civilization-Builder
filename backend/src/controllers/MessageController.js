...
class MessageController {
  ...
  async getChatUsers(req, res) {
    try {
      ...
      // Return all active users except self, with their roles
      const users = await User.findAll({
        ...
        include: [
          ...
          },
          {
            model: RegionalAdminScope,
            required: false,
            include: [{ model: State, attributes: ['id', 'state_name'] }]
          }
        ],
        limit: 100
      });
      ...
      const formatted = users.map(u => {
        ...
        const stateName = scope?.State?.state_name || null;
        const roles = u.Roles?.map(r => r.role_name) || [];
        return {
          id: u.id,
      ...
      return res.status(500).json({ success: false, message: 'Failed to fetch chat users', errors: [error.message] });
    }
  }
