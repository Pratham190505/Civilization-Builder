...
import { getSchools, approveSchool, rejectSchool, getSchoolById } from "../../api/schools";
              ...
              <tr key={s.id} className="border-t border-border hover:bg-white/5 transition">
                <td className="px-5 py-3 font-medium text-foreground">
                  <button 
                    ...
                    className="hover:underline text-blue-400 font-medium bg-transparent border-0 cursor-pointer p-0 text-left text-sm"
                  >
                    {s.school_name}
                  </button>
                </td>
                          ...
                          {schoolDetails.InspectionRequests.map((req) => (
                            <div key={req.id} className="rounded-xl border border-border p-4 bg-muted/20 space-y-3">
                              <div className="flex items-center justify-between text-xs border-b border-border pb-2">
                                ...
                                <span className="px-2 py-0.5 rounded-full font-bold bg-blue-500/10 text-blue-400">
                                  {req.status}
                                </span>
                              </div>
                          ...
                          <div className="rounded-xl border border-border p-4 bg-amber-500/5 border-amber-500/20 text-sm">
                            {schoolDetails.SchoolRankSnapshots.map((snap) => (
                              <div key={snap.id} className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                <div><div className="text-xs text-muted-foreground">Rank Tier</div><div className="text-lg font-bold text-amber-400">{snap.RankTier?.tier_name || "Bronze"}</div></div>
                                <div><div className="text-xs text-muted-foreground">Total Score</div><div className="text-lg font-bold text-foreground font-mono">{snap.total_score} pts</div></div>
                                <div><div className="text-xs text-muted-foreground">Global Rank</div><div className="text-lg font-bold text-foreground font-mono">#{snap.global_rank || "-"}</div></div>
                                <div><div className="text-xs text-muted-foreground">State Rank</div><div className="text-lg font-bold text-foreground font-mono">#{snap.state_rank || "-"}</div></div>
                                <div><div className="text-xs text-muted-foreground">District Rank</div><div className="text-lg font-bold text-foreground font-mono">#{snap.district_rank || "-"}</div></div>
                              </div>
                            ))}
                          </div>
                        )}
                              ...
                              <tbody>
                                {schoolDetails.SchoolRankHistories.map((hist) => (
                                  <tr key={hist.id} className="border-t border-border hover:bg-white/5">
                                    ...
                                    <td className="px-4 py-2 font-bold text-foreground">{hist.RankTier?.tier_name || "Bronze"}</td>
                                    <td className="px-4 py-2 font-mono">#{hist.global_rank || "-"}</td>
                                    <td className="px-4 py-2 font-mono">#{hist.state_rank || "-"}</td>
                                    <td className="px-4 py-2 font-mono">#{hist.district_rank || "-"}</td>
                                    <td className="px-4 py-2 font-mono text-foreground font-semibold">{hist.total_score} pts</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
