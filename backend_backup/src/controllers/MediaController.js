...
class MediaController {
  ...
  async submitMediaSubmission(req, res) {
    try {
      ...
      // Create Review Step (Regional Review)
      // submission_review_steps columns: id, submission_id, step_name, step_order, status, reviewed_by, reviewed_at, remarks
      await SubmissionReviewStepRepository.create({
        submission_id: submission.id,
        step_name: 'REGIONAL_REVIEW',
        step_order: 1,
        status: 'PENDING'
      });

      // Notify Regional Admins in this school's state
      const stateId = school?.District?.state_id || null;
      await notificationService.sendNotification({
        senderId: req.user.id,
        type: 'MEDIA_APPROVED',
        title: 'New Media Submission for Review',
  ...
  async reviewMediaSubmission(req, res) {
    try {
      const { submission_id, action, comments } = req.body;

      const submission = await MediaSubmissionRepository.findById(submission_id);
      if (!submission) {
        return res.status(404).json({ success: false, message: 'Submission not found', errors: [] });
      }

      const step = await SubmissionReviewStepRepository.findOne({
        where: { submission_id, step_name: 'REGIONAL_REVIEW', status: 'PENDING' }
      });

      if (!step) {
        return res.status(400).json({ success: false, message: 'No pending regional review step found', errors: [] });
      }

      // Record Review Action
      // submission_reviews columns: id, submission_id, reviewer_id, review_type, decision, comments, reviewed_at
      await SubmissionReviewRepository.create({
        submission_id,
        reviewer_id: req.user.id,
        review_type: 'REGIONAL',
        decision: action === 'APPROVE' ? 'APPROVED' : (action === 'REJECT' ? 'REJECTED' : 'RETURNED'),
        comments,
      ...
      if (action === 'APPROVE') {
        ...
        await step.update({
          ...
          reviewed_at: new Date(),
          remarks: comments || 'Approved by regional reviewer'
        });

        // Set status to REGIONAL_REVIEWED, create next step for Super Admin
        await submission.update({ status: 'REGIONAL_REVIEWED' });
        ...
        await notificationService.sendNotification({
          senderId: req.user.id,
          type: 'MEDIA_APPROVED',
          title: 'Media Awaiting Final Approval',
          message: `The post "${submission.title}" has passed regional review and awaits final Super Admin approval.`,
        ...
        await step.update({
          ...
          reviewed_by: req.user.id,
          reviewed_at: new Date(),
          remarks: comments || 'Rejected by regional reviewer'
        });
        ...
        await notificationService.sendNotification({
          senderId: req.user.id,
          type: 'MEDIA_REJECTED',
          title: 'Media Submission Rejected',
          message: `Your post "${submission.title}" was rejected during regional review.`,
        ...
        await step.update({
          ...
          reviewed_by: req.user.id,
          reviewed_at: new Date(),
          remarks: comments || 'Revisions requested'
        });

        await submission.update({ status: 'DRAFT' });

        // Notify School Admin
        await notificationService.sendNotification({
          senderId: req.user.id,
          type: 'MEDIA_REJECTED',
          title: 'Media Revisions Requested',
          message: `Changes requested on your post "${submission.title}". Status reverted to Draft.`,
          targetSchoolId: submission.school_id
        });
      }
  ...
  async approveMediaSubmission(req, res) {
    try {
      ...
      if (!step) return res.status(400).json({ success: false, message: 'No pending Super Admin review step found', errors: [] });
      ...
      await submission.update({ status: 'SUPER_APPROVED' });
      ...
      await notificationService.sendNotification({
        ...
        message: `Your post "${submission.title}" has received final approval and is ready to publish.`,
        targetSchoolId: submission.school_id
      });
  ...
  async rejectMediaSubmission(req, res) {
    try {
      ...
      await notificationService.sendNotification({
        senderId: req.user.id,
        type: 'MEDIA_REJECTED',
        title: 'Media Rejected by Super Admin',
        message: `Your post "${submission.title}" was rejected on final review.`,
        targetSchoolId: submission.school_id
      });
